...
**Status:** PARTIALLY SUPERSEDED. THE `+M` GROWING-PROJECTION FORMULA REMAINS VALID, BUT A REAL CAUCHY ROOT ALSO CONTAINS THE CONJUGATE `-M` CHARACTER, PRODUCING AN ORDER-ONE BACKWARD ORBIT SHIFT. THE OLD ONE-RATIO PRINCIPAL CLOSURE IS NOT THE CORRECT REAL-ROOT MODEL. SEE `beta21_lowu_real_root_bidirectional_principal_reset.md`.  This note closes a logical gap in `beta21_lowu_boundary_layer_bilateral_orbit_reset.md`: the two limiting shift coefficients `a_C,a_P` cannot be chosen independently because both are produced by one physical beta-zero Cauchy root `M=P-2C`.
